...
const FootballAnimation: React.FC<FootballAnimationProps> = ({
  ...
  const animate = useCallback(() => {
    ...
    if (elapsedTime < 8) {
      ...
      console.log(
        `KICK: time=${kickTime.toFixed(2)}, phase=${kickPhase.toFixed(
          3
        )}, angle=${kickAngle.toFixed(3)}`
      );
      ...
      if (kickEffectTriggered.current) {
        ...
        if (ballFlightTime >= 0) {
          ...
          const newY = Math.max(
            ...
            startY +
              vy * ballFlightTime +
              0.5 * gravity * ballFlightTime * ballFlightTime
          );
          const newZ = startZ + vz * ballFlightTime;

          ball.position.set(newX, newY, newZ);
          console.log(
            `Ball flying: ${newX.toFixed(2)}, ${newY.toFixed(
              2
            )}, ${newZ.toFixed(2)}`
          );
        }
    ...
    <div className="relative w-full h-screen">
      ...
      <div className="absolute top-4 left-4 bg-black bg-opacity-75 text-white p-4 rounded-lg">
        ...
        <div className="mt-4 text-sm">
          <p>
            <strong>Tempo:</strong> {(animationTime / 1000).toFixed(1)}s / 18.0s
          </p>
          <p>
            <strong>Estado:</strong> {isPlaying ? "Rodando" : "Parado"}
          </p>
          <p>
            <strong>Fase:</strong> {currentPhase}
          </p>
          <p>
            <strong>Progresso:</strong>{" "}
            {((animationTime / 1000 / 18) * 100).toFixed(1)}%
          </p>{" "}
          <p>
            <strong>Fase atual:</strong>{" "}
            {animationTime / 1000 < 8
              ? "1-Corrida ida/volta"
              : animationTime / 1000 < 11
              ? "2-Preparação"
              : animationTime / 1000 < 14
              ? "3-Chute"
              : "4-Comemoração"}
          </p>
        </div>
        ...
        <div className="mt-4 text-xs text-gray-300">
          ...
          <p>
            • <strong>FK:</strong> Rotação direta dos ossos (corrida/chute)
          </p>
          <p>
            • <strong>IK:</strong> Posicionamento por alvo (preparação)
          </p>
          <p>
            • <strong>Timing:</strong> Performance.now() baseado
          </p>
          <p>
            • <strong>Bones:</strong> Sistema de esqueleto com 15 ossos
          </p>
        </div>
      </div>
    </div>
